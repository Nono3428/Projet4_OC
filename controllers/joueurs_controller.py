...
class JoueursController:
    ...
    def verifier_id(self, identifiant):
        if not re.match("^[A-Za-z]{2}[0-9]{5}$", identifiant):
            return False, "L'identifiant doit comporter deux lettres suivies de cinq chiffres, comme 'AB12345'."
        
        # Vérifie l'unicité
        if identifiant in self.liste_identifiants:
            return False, "Cet identifiant existe déjà. Veuillez en choisir un autre."
